...
```bash
brew update && brew upgrade && brew tap buo/cask-upgrad && brew cu --all && brew cleanup && mas upgrade
```

注意，`brew upgrade` 不会更新通过	`--cask` 安装的软件，所以，需要 `brew tap buo/cask-upgrad && brew cu --all` 才行……
